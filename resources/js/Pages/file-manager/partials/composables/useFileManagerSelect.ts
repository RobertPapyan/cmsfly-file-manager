...
function selectMultiple(item: Directory | File, content: Content) {
  ...
  const startIndex = mergedArray.findIndex(
    (i) => i.path == firstItem.path && i.type == firstItem.type,
  );
  if (startIndex == -1) return;

  const endIndex = mergedArray.findIndex(
  ...
  selectedItems.value =
    startIndex < endIndex
      ? mergedArray.slice(startIndex, endIndex + 1)
      : mergedArray.slice(endIndex, startIndex + 1);
}

function selectIfNotSelected(item: Directory | File) {
  if (
    selectedItems.value.some((i) => i.type == item.type && i.path == item.path)
  ) {
    return;
  }
  selectedItems.value = [item];
