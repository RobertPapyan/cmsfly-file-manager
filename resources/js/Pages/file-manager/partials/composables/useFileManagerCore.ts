import { ref } from "vue"
import { Config, Content, ContentResponse, Directory, FmBaseResponse, TreeNode, TreeResponse } from "../types"
...
const tree = ref<TreeNode>(rootFolderSkeleton)


async function initFileManager(){
  const {data, error} = await useAppFetch(route('fm.initialize')).json()
...
async function getContent(disk = 'public',path = '', search = ''){
  return new Promise(async (resolve,reject)=>{
    ...
    if(search){
      isSearch.value = true
      request = await useAppFetch(route('fm.search',{disk:disk,search:search})).json<ContentResponse>()
    }else{
      isSearch.value = false
      request = await useAppFetch(route('fm.content',{disk:disk,path:path})).json<ContentResponse>()
    }
...
async function getTree(disk = 'public', path = '') {
  if(!disklist.value.includes(disk))return
  const {data, error} = await useAppFetch(route('fm.tree',{disk:disk,path:path})).json<TreeResponse>()
    if(data.value ){
      tree.value.basename = disk
      tree.value.directories = data.value.directories
      tree.value.props.hasSubdirectories = data.value.directories?.length ? true : false
    }
}

async function changeDisk(value:string){
  if(value == currentDisk.value)return
  const { data } = await useAppFetch(route('fm.select-disk',{disk:value})).json<FmBaseResponse>()
...
async function openFolderByPath(path:string) {
  ...
  const folder = findFolderInTree(path,tree.value)
  if(folder)openFolder(folder)

}

function findFolderInTree (path:string, node: TreeNode) : TreeNode | null{
  if(path == node.path){
    return node
  }
...
async function buildTree(node : TreeNode, path : string){
  return new Promise(async (resolve,reject)=>{
    if(!node.directories || !node.directories.length){
      const {data} = await useAppFetch(route('fm.tree',{disk:currentDisk.value, path:node.path})).json<TreeResponse>()
      if(data.value){
